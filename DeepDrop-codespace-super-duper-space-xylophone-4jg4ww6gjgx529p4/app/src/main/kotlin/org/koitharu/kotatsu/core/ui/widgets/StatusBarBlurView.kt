...
class StatusBarBlurView @JvmOverloads constructor(
	...
	// The blur re-record (drawing the whole target view tree into an offscreen node) is the
	// expensive part of this effect. During a fast fling it would otherwise happen on every
	// single frame; capping it to ~30fps roughly halves that cost with no visible difference,
	// since the frames in between simply redraw the previous (still-fresh-looking) blur node.
	...
	private fun drawBlur(canvas: Canvas, blurNode: RenderNode, target: View) {
		...
		val dueForRecord = now - lastRecordTimeNs >= RECORD_INTERVAL_NS || !blurNode.hasDisplayList()
		if (dueForRecord) {
			lastRecordTimeNs = now
			// Record a slightly larger slice than shown (taller below, and wider on both sides) so
			// the blur has real pixels to sample at every edge instead of clamping, which would
			// otherwise leave an under-blurred strip down the left/right of the screen and a band
			// at the fade.
	...
	private companion object {
		...
		// ~30fps cap for re-recording the blur source; the composite draw below still runs at
		// full frame rate so scrolling stays smooth, only the expensive re-capture is throttled.
		const val RECORD_INTERVAL_NS = 33_000_000L
	}
}
